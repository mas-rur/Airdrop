...
## Part 3: Deploy the airdrop app to Vercel

Same idea as Part 2, but for `airdrop-dashboard/` (the airdrop.pay3.space
app) -- and simpler, since it only ever reads public on-chain data:

```bash
cd airdrop-dashboard
vercel link          # separate Vercel project from testnet-dashboard
vercel env add NEXT_PUBLIC_RPC_URL   # same node URL as Part 2
vercel --prod
```

No faucet key needed here. Point the custom domain (`airdrop.pay3.space`)
at this project from Vercel's **Settings → Domains** once it's deployed.
